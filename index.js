...
var Rtc = require('multiserver/plugins/rtc')
...
function toSodiumKeys (keys) {
  if (!(isString(keys.public) && isString(keys.private))) { return keys }
  return {
...
function coearseAddress (address) {
  if (isObject(address)) {
    var protocol = address.source === 'rtc' ? 'rtc' : 'net'
...
module.exports = function (opts) {
  ...
  return create.use({
    ...
    init: function (api, opts, permissions, manifest) {
      ...
      if (opts.timers && !isNaN(opts.timers.handshake)) { timeout_handshake = opts.timers.handshake }

      timeout_handshake = timeout_handshake || (opts.timers ? 15e3 : 5e3)

      if (opts.timers && !isNaN(opts.timers.inactivity)) { timeout_inactivity = opts.timers.inactivity }
      ...
      if (opts.timeout) { timeout_handshake = timeout_inactivity = opts.timeout }
      ...
      var server_protocols = [
        ...
        [Rtc({port: port, host: host, server: api, id: opts.keys.id}), shs],
        [Onion({server: false}), shs]
      ]
      var client_protocols = server_protocols

      if (opts['tor-only']) { client_protocols = [[Onion({server: false}), shs]] }
      ...
      function setupRPC (stream, manf, isClient, shit) {
        ...
        if (remoteAddress.protocol === 'rtc') {
          remoteAddress.id = rpc.id
          var message = {
            hub: stream.hub,
            peer: {
